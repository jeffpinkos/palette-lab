...
export default function App() {
  ...
  return <main>
    ...
    <section id="method" className="method"><div><h2>Why these colors?</h2><p>The active engine uses the training signals supplied by this palette. The ML service compares clustering candidates, then blends cluster structure, group co-occurrence, color proximity, and your chosen mood.</p>{metadata.sourceUrl ? <a href={metadata.sourceUrl} target="_blank" rel="noreferrer">Explore {metadata.sourceName} <ArrowRight size={15} /></a> : null}</div><div className="method-visual"><div className="mini selected-mini">{lab.selected.slice(0, 3).map((color) => <i key={color.id} style={{ background: color.hex }} />)}</div><b>+</b><div className="mini">{lab.results.slice(0, 4).map((result) => <i key={result.color.id} style={{ background: result.color.hex }} />)}</div></div><aside><CircleHelp size={18} /><p>Affinity is produced by {runtime.recommendationEngine.name}. It is a recommendation score, not a probability.</p></aside></section>
